...
export default function MyWork() {
  ...
  const sections = useMemo(() => {
    ...
    for (const i of items) {
      if (i.status?.label?.toLowerCase() === 'done') continue
  ...
  return (
    <div className="my-work">
      <h2>My Work</h2>
      <p className="muted">Everything assigned to you, across all boards.</p>
      {sections.length === 0 && (
        <div className="my-work-empty">🎉 Nothing on your plate — assign items to yourself with a People column.</div>
      )}
      {sections.map(s => (
        <section key={s.key} className="my-work-section">
          ...
          {s.items.map(i => (
            <button key={i.id} className="my-work-row"
              ...
              <span className="mw-name">{i.name}</span>
